...
class DBProxy:
    ...
    def save(self, entry: dict):
        """
        Salva uma nova entrada no banco de dados (arquivo JSON).
        """
    ...
    def retrieve_top5(self):
        """
        Recupera as 5 maiores pontuações ordenadas de forma decrescente.
        """
        data = self._load_data()
        return sorted(data, key=lambda x: x['score'], reverse=True)[:5]

    def cleanup_top_scores(self, max_scores: int):
        """
        Limita o número de pontuações salvas para `max_scores` no máximo.
        """
        data = self._load_data()
